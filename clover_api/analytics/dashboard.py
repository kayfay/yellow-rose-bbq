...
def get_last_updated_time() -> str:
    ...
    try:
        conn = sqlite3.connect(DB_PATH)
        cur = conn.cursor()
        cur.execute("SELECT MAX(created_time) FROM orders")
        res = cur.fetchone()[0]
        conn.close()
        if res:
            db_time = res.replace(' ', 'T')
    except Exception:
        pass

    existing_time = None
    try:
        dash_json = ANALYTICS_DIR / "dashboard_payload.json"
        if dash_json.exists():
            with open(dash_json, 'r') as f:
                data = json.load(f)
                existing_time = data.get("forecast", {}).get("generated_at")
    except Exception:
        pass

    candidates = [t for t in [db_time, existing_time] if t]
    if candidates:
...
def generate_forecast_data_polars(days: int = 14) -> Dict[str, Any]:
    ...
    for i in range(len(future_dates)):
        ...
        # Check if closed day (Mondays are closed for pit maintenance & cleaning)
        is_closed = (day_name == "Mon" or demand_index == 0)
        if is_closed:
            base_revenue = 0.0
            brisket_raw_lbs = 0.0
            pork_lbs = 0.0
            ...
            staff_count = min(5, max(2, int(base_revenue // 1000) + 1))
    ...
    return {
        "generated_at": get_last_updated_time(),
        "days_count": days,
