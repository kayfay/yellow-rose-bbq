...
def save_items_to_sqlite_and_markdown(all_items):
    ...
    # Generate WEBSITE_MENU.md
    md_content = """# Yellow Rose BBQ - Official POS Menu & Item Catalog

This document is automatically generated from live Clover POS API data.

## Smoked Meats & BBQ Mains
| Item Name | POS Price ($) | Unit / Notes |
| :--- | :---: | :--- |
"""
    ...
    for row in item_rows:
        n = row["name"].lower()
        price_fmt = f"${row['price_usd']:.2f}" if row['price_usd'] > 0 else "Variable / Weight"
        line = f"| **{row['name']}** | {price_fmt} | {row['unit']} |"
    ...
    md_content += "## Catering, Bundles & Other\n| Item Name | POS Price ($) | Unit / Notes |\n| :--- | :---: | :--- |\n"
    md_content += "\n".join(sorted(other)) + "\n"
