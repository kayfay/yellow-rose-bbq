...
target = '    query_type = "SELECT order_type, count(DISTINCT order_id) as order_count, sum(price_usd * quantity) as total_revenue FROM order_line_items GROUP BY order_type"'
...
with open('clover_api/analytics/advanced_analytics.py', 'w') as f:
    f.write(content)
print("advanced_analytics.py patched for order_type.")
